  ...
  try {
    ...
    for (let start = 0; start < pending.length; start += BATCH_SIZE) {
      ...
      const judgements = await deps.classify({
        transactions: batch.map((transaction, offset) => brief(transaction, start + offset)),
        known,
      });

      if (!judgements.ok) {
        // The model answered and the answer was unusable. That is an outcome about this
        // batch, recorded as state -- the definition of done requires a schema failure be
        // handled rather than crash the workflow. Infrastructure failures never reach here:
        // `inferStructure` rethrows them so Inngest retries.
        return await failRun(scope, run.id, judgements.reason);
      }

      for (const judgement of judgements.value.judgements) {
        const subject = batch[judgement.index - start];
        // A judgment about a transaction that was not in the batch is not a transaction we
        // are entitled to write against. Dropping it is right: the model answering about
        // row 500 of a 40-row list has told us nothing about row 500.
        if (!subject) continue;
    ...
    await scope.update(
      reconciliationRuns,
      {
        state: "COMPLETED",
        finishedAt: new Date(),
        transactionsProcessed: pending.length,
        documentsRequired,
        ...coverage,
      },
